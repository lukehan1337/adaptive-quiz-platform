...
import streamlit as staimport os
import json
import random
import pathlib
import platform
...
def quiz_locked() -> bool:
    # 퀴즈 중인지 체크. 중간에 메뉴 이동 막으려고 만듦
    # math_submitted, voc_submitted, dict_idx로 퀴즈 종료 판단
    # dict_active, math_active, voc_active, shape_active 중 하나라도 True면 퀴즈 중
    return any([
        ...
        st.session_state.get("math_submitted") or st.session_state.get("voc_submitted")
        or st.session_state.get("shape_submitted")
        or (st.session_state.get("dict_active") and st.session_state.get("dict_idx", 0) >= 5)
    )
...
# ───────────────────────── Utilities ─────────────────────────
def load_json(path: pathlib.Path, default):
    # json 파일 읽어서 파싱. 파일 없거나 json 깨지면 default 리턴
    # 실제로 파일이 없을 수도 있어서 예외처리 필요
    if path.exists():
        try:
            # 파일 내용 읽어서 json 파싱
            return json.loads(path.read_text())
        except json.JSONDecodeError:
            # json 파싱 실패하면 그냥 default
            pass
    return default
...
def tts_play(text: str):
    # TTS로 한글 읽어주는 함수
    # gTTS로 mp3 임시파일 생성 후, OS에 따라 재생
    # macOS면 afplay, 그 외는 mpg123 사용
...
def clear_quiz_state():
    ...
    # session_state에 남아있는 퀴즈 관련 키들 싹 지움
    QUIZ_KEYS = (
        "dict_active", "math_active", "voc_active",
        "dict_questions", "dict_answers", "dict_idx",
        "math_questions", "math_answers",
        "voc_pairs", "voc_answers",
        "math_submitted", "voc_submitted",
    )
    # session_state에 있는 키들 중 퀴즈 관련된 것만 삭제
    for k in list(st.session_state.keys()):
        if k.startswith(("dict_", "math_", "voc_")) or k in QUIZ_KEYS:
            del st.session_state[k]
    # username은 남기고, 페이지/라디오만 초기화
    st.session_state.pop("page", None)
    st.session_state.pop("nav", None)
...
def get_user(name: str):
    ...
    if name not in USERS:
        # 유저 없으면 기본 구조로 생성
        USERS[name] = {"dict": [], "math": [], "vocab": [], "history": []}
        save_json(DATA_FILE, USERS)
    user = USERS[name]
    ...
    if not user.get("history"):
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        # dict, math, vocab 각각의 점수 리스트를 history로 옮김
        for quiz_key in ("dict", "math", "vocab"):
            for s in user.get(quiz_key, []):
                user["history"].append(
                    {
                        "time": timestamp,
...
def quiz_locked() -> bool:
    # 퀴즈 중인지 체크. 중간에 메뉴 이동 막으려고 만듦
    # math_submitted, voc_submitted, dict_idx로 퀴즈 종료 판단
    # dict_active, math_active, voc_active 중 하나라도 True면 퀴즈 중
    return any([
        st.session_state.get("dict_active", False),
        st.session_state.get("math_active", False),
        st.session_state.get("voc_active", False),
...
def main():
    ...
    if not lock:
        # 사이드바 메뉴. 대시보드/받아쓰기/수학/단어/로그아웃
        st.session_state.page = st.sidebar.radio(
            "메뉴",
            ["대시보드", "받아쓰기", "수학", "단어", "로그아웃"],
            key="nav",
            help="메뉴를 선택하세요",
        )
    else:
...
def show_dashboard(user):
    ...
    best_vocab = max(user["vocab"] or [0])
    # 평균 점수 계산
    avg_best = round((best_dict + best_math + best_vocab) / 3, 3)
    ...
    if history:
        ...
        for h in history:
            # 날짜 파싱 실패하면 원본 출력
            try:
                ts = datetime.datetime.strptime(h["time"].replace("‑", "-"), "%Y-%m-%d %H:%M:%S").strftime("%Y-%m-%d %H:%M:%S")
            except ValueError:
                ts = h["time"]
            table_data["시간"].append(ts)
            table_data["시험 종류"].append(h["type"])
            table_data["정답률(%)"].append(h["score"])
        st.table(table_data)
